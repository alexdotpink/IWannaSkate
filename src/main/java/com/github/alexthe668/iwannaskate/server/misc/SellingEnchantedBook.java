...
public class SellingEnchantedBook implements VillagerTrades.ItemListing {
    ...
    public SellingEnchantedBook(Holder<Enchantment> enchantment, int price, int maxUses, int xpValue) {
    ...
    public MerchantOffer getOffer(Entity tradingWith, RandomSource randomSource) {
        int i = Mth.nextInt(randomSource, enchantment.value().getMinLevel(), enchantment.value().getMaxLevel());
        ...
        return new MerchantOffer(new ItemCost(Items.EMERALD, this.price), itemstack, this.maxUses, this.xpValue, this.priceMultiplier);
    }
}
